...
static ExecResult dispatch(TokenList *tl, Value *piped) {
    ...
    if (strcmp(cmd, "auth") == 0) {
        ...
        if (!g_current_user[0]) { rush_error("not logged in"); r.ok = 1; return r; }
        if (!g_auth_verified) {
            if (verify_current_user_password() != 0) { r.ok = 1; return r; }
            g_auth_verified = 1;
        }
        ...
        int prev_via_auth = g_via_auth;
        g_via_auth = 1;
        ExecResult inner_r = dispatch(&inner, piped);
        g_via_auth = prev_via_auth;
        return inner_r;
    }
    ...
    if (strcmp(cmd, "lib") == 0)     return cmd_lib(tl);
    if (strcmp(cmd, "netch") == 0)   return cmd_netch(args, argc);
    if (strcmp(cmd, "monitor") == 0) return cmd_monitor(args, argc, piped);
    if (strcmp(cmd, "launch") == 0)  return cmd_launch(tl);
    if (strcmp(cmd, "config") == 0)  return cmd_config(tl);
    if (strcmp(cmd, "pause") == 0)   return cmd_pause();
    if (strcmp(cmd, "sdown") == 0)   return cmd_sdown(tl);
    if (strcmp(cmd, "frmt") == 0)    return cmd_frmt(tl);
    if (strcmp(cmd, "partcre") == 0) return cmd_partcre(tl);
    if (strcmp(cmd, "partdel") == 0) return cmd_partdel(tl);
    if (strcmp(cmd, "partres") == 0) return cmd_partres(tl);
    if (strcmp(cmd, "kill") == 0) {
        if (tl->count >= 3 && strcmp(tl->tokens[1], "pros") == 0) return cmd_kill_pros(tl->tokens[2]);
        rush_error("kill expects: kill pros <n>");
    ...
    if (strcmp(cmd, "list") == 0) {
        ...
        if (tl->count >= 2 && strcmp(tl->tokens[1], "pros") == 0) return cmd_list_pros();
        if (tl->count >= 2 && strcmp(tl->tokens[1], "dsk") == 0) return cmd_list_dsk();
        if (tl->count >= 2 && strcmp(tl->tokens[1], "lab") == 0) return cmd_list_lab();
        if (tl->count >= 2 && strcmp(tl->tokens[1], "part") == 0) {
            if (tl->count < 3) {
                rush_error("list part expects a disk number, e.g. list part 1");
                ExecResult r = {1, {VAL_INT,0,NULL}, 0};
                return r;
            }
            return cmd_list_part(tl->tokens[2]);
        }
...
void run_program(char **lines, int n) {
    int pc = 0;
    char word[32];
    while (pc < n && g_running && !g_script_stop && !g_interrupt) {
        ...
        if (strcmp(word, "loop") == 0) {
            ...
            /* "loop" with no number after it is an infinite loop - runs
             * until 'quit'/'exit' inside the body, or Ctrl+C, stops it. */
            int infinite = (*rest == '\0');
            int count = infinite ? 0 : atoi(rest);
            ...
            for (int c = 0; (infinite || c < count) && g_running && !g_script_stop && !g_interrupt; c++) {
