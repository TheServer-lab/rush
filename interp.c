...
static ExecResult dispatch(TokenList *tl, Value *piped) {
    ...
    if (strcmp(cmd, "list") == 0)  return cmd_list(args, argc, tl, piped);
    if (strcmp(cmd, "read") == 0)  return cmd_read(args, argc, piped);
    if (strcmp(cmd, "about") == 0) return cmd_about(args, argc, piped);
    if (strcmp(cmd, "del") == 0)   return cmd_del(args, argc, tl, piped);
    ...
    if (strcmp(cmd, "open") == 0 || strcmp(cmd, "edit") == 0 ||
        strcmp(cmd, "dload") == 0 || strcmp(cmd, "extr") == 0) {
...
ExecResult exec_line(const char *raw) {
    ...
    if (probe.count >= 3 && strcmp(probe.tokens[1], "=") == 0 &&
        ...
        if (probe.count == 3 && probe.tokens[2][0] == '"') {
            ...
            /* single bare word on the right of '=' with no command
             * meaning: treat as a plain string literal, e.g. a = hello */
            var_set_string(name, probe.tokens[2]);
        } else {
            /* rest is a sub-command; find where it starts in the raw string */
            const char *p = strstr(trimmed, "= ");
            const char *rest = p ? p + 2 : trimmed;
            ExecResult sub = exec_line(rest);
            if (sub.has_value) {
