...
/* ---------- launch targets (config launch <name> = <path>) ---------- */
LaunchTarget *launch_target_find(const char *name) {
    for (int i = 0; i < RUSH_MAX_LAUNCH_TARGETS; i++) {
        if (g_launch_targets[i].in_use && strcmp(g_launch_targets[i].name, name) == 0) return &g_launch_targets[i];
    }
    return NULL;
}
...
void launch_target_save_current(void) {
    char path[1100];
    build_launcher_path(path, sizeof(path));
    ensure_user_dirs();
    FILE *f = fopen(path, "w");
    if (!f) return;
    for (int i = 0; i < RUSH_MAX_LAUNCH_TARGETS; i++) {
        if (g_launch_targets[i].in_use) {
            fprintf(f, "%s\t%s\n", g_launch_targets[i].name, g_launch_targets[i].path);
        }
    }
    fclose(f);
}
...
void launch_target_set(const char *name, const char *path) {
    LaunchTarget *t = launch_target_find(name);
    if (!t) {
        for (int i = 0; i < RUSH_MAX_LAUNCH_TARGETS; i++) {
            if (!g_launch_targets[i].in_use) { t = &g_launch_targets[i]; break; }
        }
    }
    if (!t) return; /* table full */
    t->in_use = 1;
    strncpy(t->name, name, sizeof(t->name) - 1);
    t->name[sizeof(t->name) - 1] = '\0';
    strncpy(t->path, path, sizeof(t->path) - 1);
    t->path[sizeof(t->path) - 1] = '\0';
    launch_target_save_current();
}

void launch_target_clear_all(void) {
    for (int i = 0; i < RUSH_MAX_LAUNCH_TARGETS; i++) g_launch_targets[i].in_use = 0;
}

void launch_target_load_current(void) {
    launch_target_clear_all();
    char path[1100];
    build_launcher_path(path, sizeof(path));
    FILE *f = fopen(path, "r");
    if (!f) return;
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!tab) continue;
        *tab = '\0';
        launch_target_set(line, tab + 1); /* re-saves each line, harmless, matches alias_load_current */
    }
    fclose(f);
}
