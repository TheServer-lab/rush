...
void build_config_path(char *out, size_t sz) {
    if (g_current_user[0]) {
        snprintf(out, sz, "%s/rush_data/user/%s/config.txt", g_start_dir, g_current_user);
    } else {
        snprintf(out, sz, "%s/rush_data/config.txt", g_start_dir);
    }
}

void config_save_current(void) {
    ensure_user_dirs();
    char path[1100];
    build_config_path(path, sizeof(path));
    FILE *f = fopen(path, "w");
    if (!f) return;
    fprintf(f, "default_tier=%s\n", g_default_tier);
    fprintf(f, "package_backend=%s\n", g_package_backend);
    fprintf(f, "lib_backend=%s\n", g_lib_backend);
    fclose(f);
}

void config_load_current(void) {
    /* reset to defaults first, in case this scope has no config.txt yet */
    strncpy(g_default_tier, "member", sizeof(g_default_tier) - 1);
    g_default_tier[sizeof(g_default_tier) - 1] = '\0';
    g_package_backend[0] = '\0';
    g_lib_backend[0] = '\0';
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        if (strcmp(key, "default_tier") == 0 && value[0]) {
            strncpy(g_default_tier, value, sizeof(g_default_tier) - 1);
            g_default_tier[sizeof(g_default_tier) - 1] = '\0';
        } else if (strcmp(key, "package_backend") == 0) {
            strncpy(g_package_backend, value, sizeof(g_package_backend) - 1);
            g_package_backend[sizeof(g_package_backend) - 1] = '\0';
        } else if (strcmp(key, "lib_backend") == 0) {
            strncpy(g_lib_backend, value, sizeof(g_lib_backend) - 1);
...
void build_launcher_path(char *out, size_t sz) {
    if (g_current_user[0]) {
        snprintf(out, sz, "%s/rush_data/user/%s/launcher.txt", g_start_dir, g_current_user);
    } else {
        snprintf(out, sz, "%s/rush_data/launcher.txt", g_start_dir);
    }
}
...
/* Re-prompts the currently logged-in user for their password and checks
 * it against the stored hash, the way sudo does on the first use in a
 * session. Returns 0 if it matches, 1 otherwise (prints its own error).
 * Does not touch g_current_user/g_current_role either way. */
int verify_current_user_password(void) {
    if (!g_current_user[0]) { printf("error not logged in\n"); return 1; }

    char role[RUSH_MAX_ROLE], salt[17], hash[65];
    if (!find_user_line(g_current_user, role, salt, hash)) {
        printf("error invalid username or password\n");
        return 1;
    }

    printf("[auth] password for %s: ", g_current_user);
    fflush(stdout);
    char password[256];
    read_hidden_line(password, sizeof(password));
    ...
    if (strcmp(attempt_hash, hash) != 0) {
        log_action(g_current_user, "failed auth verification");
        printf("error invalid username or password\n");
        return 1;
    }
    return 0;
...
int do_promo(const char *actor_role, const char *target_user, const char *new_role) {
    ...
    /* Anti-seizure rule: an admin can never demote a *different* admin.
     * Without this, admin A could promote B to admin, and B could
     * immediately demote A, seizing sole control. Self-demotion is
     * still allowed (subject to the last-admin check below), since
     * stepping yourself down is a choice, not a takeover. */
    char cur_role[RUSH_MAX_ROLE] = "", cur_salt[17], cur_hash[65];
    if (find_user_line(target_user, cur_role, cur_salt, cur_hash) &&
        strcmp(cur_role, "admin") == 0 &&
        strcmp(new_role, "admin") != 0 &&
        strcmp(target_user, g_current_user) != 0) {
        printf("error cannot demote another admin\n");
        return 1;
    }
    if (strcmp(cur_role, "admin") == 0 && strcmp(new_role, "admin") != 0 &&
        count_admins() <= 1) {
...
int do_cali(const char *actor_role, const char *target_user) {
    if (!g_current_user[0]) { printf("error not logged in\n"); return 1; }
    if (!actor_role || strcmp(actor_role, "admin") != 0) { printf("error insufficient permission\n"); return 1; }
    if (!user_exists(target_user)) { printf("error user %s not found\n", target_user); return 1; }

    char path[1100];
    snprintf(path, sizeof(path), "%s/rush_data/user/%s/alias.txt", g_start_dir, target_user);
    if (remove(path) != 0) {
        printf("error %s has no saved aliases\n", target_user);
        return 1;
    }
    char logmsg[256];
    snprintf(logmsg, sizeof(logmsg), "cleared aliases for %s", target_user);
    log_action(g_current_user, logmsg);
    printf("cleared aliases for %s\n", target_user);
    return 0;
}
