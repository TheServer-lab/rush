...
```
gcc -Wall -Wextra -o rush main.c interp.c commands.c vars.c net.c users.c sha256.c image.c
```

## Build (Windows, MinGW)
```
gcc -Wall -Wextra -o rush.exe main.c interp.c commands.c vars.c net.c users.c sha256.c image.c -lws2_32
```
...
gcc -Wall -Wextra -o rush.exe main.c interp.c commands.c vars.c net.c users.c sha256.c image.c rush_res.o -lws2_32
```
...
- `view <image.bmp/.ppm>` displays an image right in the terminal,
  using ANSI truecolor half-block rendering (two source pixel rows
  per printed terminal row, downscaled to fit the terminal size).
  Fully hand-written, zero dependencies - deliberately scoped to
  uncompressed BMP (24/32-bit) and binary PPM (P6) only, since real
  PNG/JPEG decoding is a much bigger undertaking not worth attempting
  casually. `open` also refuses `.bmp`/`.ppm` (and other binary
  types) as text, same as before.
